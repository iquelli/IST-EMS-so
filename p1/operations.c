...
int ems_terminate() {
  ...
  if (pthread_rwlock_destroy(&event_list->lock_list) != 0) {
...
// create - cria evento
// reserve - reservar lugar (convém pôr write e read block)
// show - mostra event (convẽm pôr write block)
// list - faz uma lista do event (convẽm pôr read block)
